...
export function AppSidebar() {
  const { tracks } = useGlobalAudio();

  return (
    <Sidebar collapsible="icon">
      <SidebarHeader>
        <div className="p-4 border-b flex items-center justify-between group-data-[collapsible=icon]:justify-center">
          <h2 className="font-semibold text-lg group-data-[collapsible=icon]:hidden">Audio Editor</h2>
        ...
        <SidebarGroup>
          ...
          <div className="group-data-[collapsible=icon]:block hidden flex flex-col gap-2">
            {tracks.map((track, index) => (
              <div key={track.id} className="w-8 h-8 bg-accent rounded flex items-center justify-center text-accent-foreground hover:bg-accent/80 cursor-pointer transition-colors" title={track.title}>
